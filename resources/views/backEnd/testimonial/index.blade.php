            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('testimonials.create') }}" class="btn btn-primary rounded-pill">Create</a>
                    </div>
                    <h4 class="page-title">Testimonial Manage</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
                            ...
                            <tbody>
                                @foreach ($data as $key => $value)
                                    <tr>
                                        ...
                                        <td><img src="{{ asset($value->image) }}" style="height: 40px;width: auto;" alt=""></td>
                                        <td>
                                            @if ($value->status == 1)
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 1)
                                                    <form method="post" action="{{ route('testimonials.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('testimonials.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
