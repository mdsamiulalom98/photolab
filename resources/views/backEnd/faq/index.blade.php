        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{route('faqs.create')}}" class="btn btn-primary rounded-pill">Create</a>
                </div>
                <h4 class="page-title">FAQ Manage</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="button-list">
                                    @if($value->status == 1)
                                    <form method="post" action="{{route('faqs.inactive')}}" class="d-inline">
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('faqs.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
                                    ...
                                    <a href="{{route('faqs.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                    <form method="post" action="{{route('faqs.destroy')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
