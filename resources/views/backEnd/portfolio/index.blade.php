            ...
            <div class="col-12">
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('portfolios.create') }}" class="btn btn-primary rounded-pill">Create</a>
                    </div>
                            ...
                            <tbody>
                                @foreach ($data as $key => $value)
                                    <tr>
                                        ...
                                        <td><img src="{{ asset($value->image_one) }}" style="height: 60px; width: auto"></td>
                                        <td><img src="{{ asset($value->image_two) }}" style="height: 60px; width: auto"></td>
                                        <td>
                                            @if ($value->status == 1)
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 1)
                                                    <form method="post" action="{{ route('portfolios.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('portfolios.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <a href="{{ route('portfolios.edit', $value->id) }}"
                                                    class="btn btn-xs btn-primary waves-effect waves-light"><i
                                                        class="fe-edit-1"></i></a>

                                                <form method="post" action="{{ route('portfolios.destroy') }}"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                    <button type="submit"
