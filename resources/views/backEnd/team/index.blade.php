        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{route('teams.create')}}" class="btn btn-primary waves-effect waves-light btn-sm rounded-pill">Create</a>
                </div>
                <h4 class="page-title">Team Manage </h4>
            </div>
        </div>
    </div>       
    <!-- end page title --> 
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="button-list">
                                    @if($value->status == 1)
                                    <form method="post" action="{{route('teams.inactive')}}" class="d-inline"> 
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('teams.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">        
                                    ...
                                    <a href="{{route('teams.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                    <form method="post" action="{{route('teams.destroy')}}" class="d-inline">        
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
